...
fn una_rafaga_deja_los_buffers_acotados_y_no_pierde_regalos() {
    ...
    assert!(
        snapshot.gift_ranking.len() <= 10,
        "el ranking de quien aporta esta acotado: {}",
        snapshot.gift_ranking.len()
    );
...
///
/// **Los contadores se leen antes de cerrar, esperando a que avancen.** No es un
/// detalle: `on_event` solo **encola**, y el que escribe es un hilo aparte. Mirar
/// el snapshot inmediatamente despues de publicar era una carrera; en una maquina
/// rapida el hilo llegaba a procesar algo y la prueba pasaba, pero en un runner de
/// CI cargado todavia no habia avanzado nada y los dos contadores valian cero, asi
/// que fallaba por la velocidad del equipo y no por un fallo del motor (fue
/// exactamente el fallo del CI). Y hay que leer **antes** de `shutdown()` porque
/// `shutdown()` retira el escritor (`AppState::shutdown` hace `take`), con lo que
/// a partir de ahi el snapshot informa ceros por diseno.
///
/// La espera es por plazo acotado, nunca un `sleep` fijo de adorno: en cuanto el
/// escritor hace su primer trabajo, o el primer descarte, la condicion se cumple.
#[test]
fn el_escritor_reporta_su_trabajo_y_sus_descartes() {
    ...
    // Espera acotada a que el escritor (o el descarte) se manifieste.
    let inicio = Instant::now();
    let en_curso = loop {
        let foto = estado.snapshot();
        if foto.db_written + foto.db_dropped > 0 || inicio.elapsed() >= ESPERA_MAXIMA {
            break foto;
        }
    ...
    assert!(
        ...
        "el escritor deberia haber recibido trabajo en {ESPERA_MAXIMA:?}: \
         escritos={}, descartados={}",
        en_curso.db_written,
        en_curso.db_dropped
    );
    assert!(
