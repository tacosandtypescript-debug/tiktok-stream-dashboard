...
/// (id, @handle, apodo, foto de perfil).
///
/// El avatar hace falta aqui porque el panel y el overlay pintan la foto de cada
/// persona en las tablas de ranking: sin avatar en el guion, esas vistas no se
/// podrian probar sin gastar cuota contra un directo real. Es el mismo criterio
/// que ya usa el simulador con los regalos y las rachas.
const NOMBRES: [(&str, &str, &str, &str); 6] = [
    (
        "1",
        "carlos123",
        "Carlos",
    ...
    ("2", "maria_g", "María", "https://i.pravatar.cc/64?img=45"),
    ("3", "juanpe", "Juan", "https://i.pravatar.cc/64?img=33"),
    ("4", "ana.stream", "Ana", "https://i.pravatar.cc/64?img=5"),
    ("5", "pedro99", "Pedro", "https://i.pravatar.cc/64?img=68"),
    ("6", "sofia.ttv", "Sofía", "https://i.pravatar.cc/64?img=23"),
];

/// Usuario del guion a partir de su indice.
fn usuario(indice: usize) -> UserRef {
    let (id, unique_id, nickname, avatar_url) = NOMBRES[indice % NOMBRES.len()];
    UserRef {
        id: id.to_string(),
        ...
        Box::pin(async move {
            ...
            self.bus.publish(
                ...
                EventKind::StreamConnected {
                    ...
                    // El simulador no tiene perfil de TikTok que enseñar: la ficha
                    // se queda sin datos y la cabecera pinta el @usuario.
...
impl Guion {
    fn step(&self, tick: u64) {
        let user = usuario(tick as usize);
    ...
    fn gift(&self, tick: u64, gift_index: usize, repeat: i32, final_: bool) {
        ...
        // Los regalos los manda el mismo par de usuarios, como en un directo.
        let user = usuario(tick as usize % 2);
        self.bus.publish(
