...
use crate::core::event::{GiftInfo, Perfil, UserRef};
...
struct RoomInfo {
    ...
    live: bool,
    title: String,
    /// Perfil del duenio. Puede venir vacio: la sala se resuelve igual.
    perfil: Perfil,
}
...
impl Supervisor {
    ...
    async fn run(&self, mut cancel: watch::Receiver<bool>) {
        ...
        loop {
            ...
            // El perfil es un adorno: si TikTok no lo manda (campos renombrados,
            // respuesta recortada) la sala se resuelve igual y solo se pierde la
            // ficha. Se deja constancia para poder diagnosticarlo.
            if room.perfil.esta_vacio() {
                tracing::debug!(handle = %self.handle, "la respuesta no trajo perfil del duenio");
            }
...
/// Nombres candidatos de cada dato del perfil, en orden de preferencia.
///
/// Los del esquema web (`nickname`, `uniqueId`, `signature`, `avatarLarger`,
/// `stats.*`) estan contrastados con la implementacion de referencia del
/// ecosistema (Evil0ctal/Douyin_TikTok_Download_API, `author_from_web_node`):
/// ahi los contadores cuelgan de `stats` o de `statsV2` (los mismos nombres,
/// como texto) y viven **al lado** del usuario, no dentro de el, que es justo
/// por lo que la busqueda admite varios objetos. El esquema aweme de los
/// comentarios usa snake_case (`unique_id`, `avatar_larger`), asi que va de
/// respaldo.
///
/// No hay captura real de la sala y TikTok los cambia: se prueban todas las
/// rutas y, si una desaparece, se pierde solo ese dato, nunca la ficha entera.
const RUTAS_APODO: [&str; 1] = ["nickname"];
const RUTAS_UNIQUE_ID: [&str; 2] = ["uniqueId", "unique_id"];
const RUTAS_AVATAR: [&str; 5] = [
    "avatarLarger",
    "avatarMedium",
    "avatarThumb",
    "avatar_larger",
...
const RUTAS_BIO: [&str; 1] = ["signature"];
const RUTAS_SEGUIDORES: [&str; 4] = [
    "stats.followerCount",
    // `statsV2` repite los contadores como texto para no perderlos por encima
    // del entero seguro de JavaScript.
    "statsV2.followerCount",
    "followInfo.followerCount",
    "followerCount",
];
const RUTAS_SEGUIDOS: [&str; 4] = [
    "stats.followingCount",
    "statsV2.followingCount",
    "followInfo.followingCount",
    "followingCount",
];
const RUTAS_LIKES: [&str; 4] = [
    // `heartCount` son los likes recibidos; `diggCount` son los dados, que no es
...
/// Busca una ruta con puntos (`stats.followerCount`) en el primer objeto que la
/// tenga.
///
/// Los objetos van del mas concreto al mas general: segun la via, algunos
/// contadores cuelgan del contenedor del usuario (`data`) y no del usuario.
fn buscar_en_objetos<'a>(objetos: &[&'a Value], ruta: &str) -> Option<&'a Value> {
    let puntero = format!("/{}", ruta.replace('.', "/"));
    objetos.iter().find_map(|objeto| objeto.pointer(&puntero))
}

/// Primer texto no vacio de las rutas candidatas.
fn texto_de(objetos: &[&Value], rutas: &[&str]) -> String {
    rutas
        .iter()
        .filter_map(|ruta| buscar_en_objetos(objetos, ruta))
        .filter_map(Value::as_str)
        .find(|texto| !texto.is_empty())
        .unwrap_or_default()
        .to_string()
}

/// Primer contador legible de las rutas candidatas.
///
/// Acepta numero JSON y texto porque la misma respuesta llega de las dos formas
/// segun la via (HTML o API). Lo que no se pueda leer como numero se descarta y
/// se prueba la siguiente ruta: un contador ausente no puede inventarse.
fn numero_de(objetos: &[&Value], rutas: &[&str]) -> Option<u64> {
    for ruta in rutas {
        let Some(valor) = buscar_en_objetos(objetos, ruta) else {
            continue;
        };
        let numero = match valor {
            Value::Number(numero) => numero.as_u64().or_else(|| {
                // Algunos contadores llegan con decimales; se redondea hacia abajo.
                numero
                    .as_f64()
                    .filter(|valor| valor.is_finite() && *valor >= 0.0)
                    .map(|valor| valor as u64)
            }),
            Value::String(texto) => texto.trim().parse::<u64>().ok(),
            _ => None,
        };
        if numero.is_some() {
            return numero;
...
/// Saca el perfil del duenio de la sala de la respuesta cruda de TikTok.
///
/// Es pura y no puede fallar: devuelve un `Perfil` vacio cuando la respuesta no
/// trae nada reconocible. `objetos` son los sitios donde puede estar el perfil,
/// del mas concreto al mas general (`user`, su contenedor y la raiz).
fn extraer_perfil(objetos: &[&Value]) -> Perfil {
    Perfil {
        unique_id: texto_de(objetos, &RUTAS_UNIQUE_ID),
        nickname: texto_de(objetos, &RUTAS_APODO),
        avatar: texto_de(objetos, &RUTAS_AVATAR),
        bio: texto_de(objetos, &RUTAS_BIO),
        seguidores: numero_de(objetos, &RUTAS_SEGUIDORES),
        seguidos: numero_de(objetos, &RUTAS_SEGUIDOS),
        likes: numero_de(objetos, &RUTAS_LIKES),
    }
}
...
fn parse_live_html(html: &str) -> Result<RoomInfo> {
    ...
    Ok(RoomInfo {
        ...
        title: json
            ...
            .unwrap_or("")
            .to_string(),
        perfil: extraer_perfil(&[user, info, &json]),
    })
}
...
async fn resolve_room_api(
    ...
    // El perfil puede estar en `data.user` o colgando del propio `data`; se
    // prueban los dos y, como ultimo recurso, la raiz de la respuesta. El orden
    // importa: lo mas concreto primero.
    let mut donde_buscar: Vec<&Value> = Vec::new();
    if let Some(user) = json.pointer("/data/user") {
        donde_buscar.push(user);
    }
    if let Some(data) = json.get("data") {
        donde_buscar.push(data);
    }
    donde_buscar.push(&json);
    let perfil = extraer_perfil(&donde_buscar);
...
mod tests {
    ...
    /// La foto de perfil sale del `avatar_thumb` (tag 9) y **sobrevive al cable**.
    ///
    /// La prueba no se conforma con rellenar el struct: codifica el mensaje,
    /// lo vuelve a decodificar y comprueba que la URL sigue ahi. Asi se cubre lo
    /// unico que puede fallar de verdad con un tag nuevo escrito a mano (que el
    /// numero no sea el que TikTok usa), que es donde el proyecto ya se ha
    /// equivocado antes: el tag correcto sale de la metadata instalada, no de un
    /// `.proto` de terceros.
    #[test]
    fn el_avatar_del_usuario_sobrevive_al_cable() {
        let usuario = User {
            id: 7,
            nickname: "Carlos".to_string(),
            display_id: "carlos123".to_string(),
            avatar_thumb: Some(ImageModel {
                url_list: vec!["https://ejemplo/thumb.jpg".to_string()],
                uri: "thumb-uri".to_string(),
                height: 64,
                width: 64,
            }),
            ..Default::default()
        ...
        let bytes = usuario.encode_to_vec();
        let decodificado = User::decode(bytes.as_slice()).expect("el usuario debe decodificar");
        let referencia = user_ref(&Some(decodificado)).expect("deberia haber usuario");

        assert_eq!(
            referencia.avatar_url, "https://ejemplo/thumb.jpg",
            "el avatar deberia salir del tag 9 y llegar intacto"
        );
        assert_eq!(referencia.unique_id, "carlos123");
    }

    /// Sin avatar el contrato no cambia: la cadena queda vacia y el campo ni
    /// siquiera se serializa (`skip_serializing_if`), asi que los eventos de
    /// quien no tiene foto siguen teniendo la misma forma que antes.
    #[test]
    fn sin_avatar_el_campo_no_aparece_en_el_json() {
        let referencia = user_ref(&Some(usuario(9, "sin_foto"))).expect("deberia haber usuario");
        assert!(referencia.avatar_url.is_empty(), "no hay foto que poner");

        let json = serde_json::to_string(&referencia).expect("deberia serializar");
        assert!(
            !json.contains("avatar_url"),
            "sin foto el campo no debe viajar: {json}"
        );
    }
    ...
    fn la_via_html_resuelve_la_sala_y_detecta_el_fin_del_directo() {
        ...
        assert_eq!(room.perfil.unique_id, "carlos", "el duenio va en el perfil");
        assert_eq!(
            room.perfil.seguidores, None,
            "sin contadores en la respuesta, ninguno se inventa"
        );
        ...
        assert!(
            room.perfil.esta_vacio(),
            "un perfil vacio no puede impedir resolver la sala"
        );
    ...
    /// El perfil completo: los nombres que hoy usa TikTok, todos a la vez.
    #[test]
    fn el_perfil_se_extrae_completo_del_usuario() {
        let user = serde_json::json!({
            "uniqueId": "carlos",
            "nickname": "Carlos",
            "avatarLarger": "https://cdn.example/grande.jpg",
            "avatarMedium": "https://cdn.example/medio.jpg",
            "avatarThumb": "https://cdn.example/mini.jpg",
            "signature": "jugando todas las noches",
            "stats": {
                "followerCount": 1702,
                "followingCount": 33,
                "heartCount": 98765
        ...
        let perfil = extraer_perfil(&[&user]);
        assert_eq!(perfil.unique_id, "carlos");
        assert_eq!(perfil.nickname, "Carlos");
        assert_eq!(
            perfil.avatar, "https://cdn.example/grande.jpg",
            "el avatar grande es el preferido"
        );
        assert_eq!(perfil.bio, "jugando todas las noches");
        assert_eq!(perfil.seguidores, Some(1702));
        assert_eq!(perfil.seguidos, Some(33));
        assert_eq!(perfil.likes, Some(98765));
    }

    /// Con solo el apodo y el avatar, los tres contadores quedan en `None` y la
    /// extraccion no falla: un cero pintado seria una mentira.
    #[test]
    fn sin_contadores_el_perfil_los_deja_en_none() {
        let user = serde_json::json!({
            "nickname": "Ana",
            "avatarMedium": "https://cdn.example/ana.jpg"
        });
        let perfil = extraer_perfil(&[&user]);
        assert_eq!(perfil.nickname, "Ana");
        assert_eq!(perfil.avatar, "https://cdn.example/ana.jpg");
        assert_eq!(perfil.seguidores, None);
        assert_eq!(perfil.seguidos, None);
        assert_eq!(perfil.likes, None);
        assert_eq!(perfil.unique_id, "", "lo que no llega queda vacio");
        assert!(!perfil.esta_vacio(), "con apodo ya hay ficha que pintar");
    }

    /// La misma respuesta llega con los contadores como numero o como texto.
    ///
    /// `statsV2` es el contenedor que TikTok manda con los mismos nombres pero
    /// como texto, para no perderlos por encima del entero seguro de JavaScript.
    #[test]
    fn los_contadores_en_texto_tambien_se_leen() {
        let user = serde_json::json!({"unique_id": "ana"});
        let contenedor = serde_json::json!({
            "statsV2": {
                "followerCount": "1702",
                "followingCount": "33",
                "heartCount": "98765"
        ...
        let perfil = extraer_perfil(&[&user, &contenedor]);
        assert_eq!(
            perfil.unique_id, "ana",
            "el nombre alternativo tambien vale"
        );
        assert_eq!(perfil.seguidores, Some(1702));
        assert_eq!(perfil.seguidos, Some(33));
        assert_eq!(perfil.likes, Some(98765));

        // Los nombres mas viejos, tambien en texto.
        let viejo = serde_json::json!({
            "followInfo": {"followerCount": "12", "followingCount": "3"},
            "heart": "45"
        });
        let perfil = extraer_perfil(&[&viejo]);
        assert_eq!(perfil.seguidores, Some(12));
        assert_eq!(perfil.seguidos, Some(3));
        assert_eq!(perfil.likes, Some(45));

        // Un texto que no es un numero no se convierte en cifra: se descarta.
        let roto = serde_json::json!({"stats": {"heartCount": "1,2 K"}});
        assert_eq!(numero_de(&[&roto], &RUTAS_LIKES), None);
    }

    /// Algunos contadores cuelgan del contenedor del usuario, no del usuario.
    #[test]
    fn los_contadores_se_buscan_tambien_en_el_contenedor() {
        // Forma real de `/api/user/detail/`: el usuario en `user` y sus
        // contadores **al lado**, en `stats`. La sala puede venir igual.
        let user = serde_json::json!({
            "uniqueId": "carlos",
            "avatar_thumb": "https://cdn.example/ana.jpg"
        });
        let contenedor = serde_json::json!({
            "user": {"uniqueId": "carlos"},
            "stats": {
                "followerCount": 1702,
                "followingCount": 33,
                "heartCount": 98765
        ...
        assert_eq!(perfil.likes, Some(98765));
        assert_eq!(
            perfil.avatar, "https://cdn.example/ana.jpg",
            "el avatar en snake_case (esquema aweme) tambien se acepta"
        );
    }

    /// Una respuesta sin nada reconocible no rompe: perfil vacio, sin panico.
    #[test]
    fn una_respuesta_sin_perfil_devuelve_un_perfil_vacio() {
        let json = serde_json::json!({"data": {"liveRoom": {"status": 2}}});
        let perfil = extraer_perfil(&[&json]);
        assert!(perfil.esta_vacio());
        assert_eq!(perfil, Perfil::default());
    }
