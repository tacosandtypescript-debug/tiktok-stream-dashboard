...
Push-Location $desktop
try {
    # El sidecar empaquetado y los avisos de terceros viven en
    # `tauri.bundle.conf.json`, que solo se aplica aqui: asi `cargo build` y
    # `cargo test` funcionan en un clon limpio, sin el ejecutable del sidecar.
    & node 'node_modules\@tauri-apps\cli\tauri.js' build --no-bundle --ci --config 'src-tauri/tauri.bundle.conf.json'
...
if ($Bundle) {
    ...
    Push-Location $desktop
    try {
        & node $tauriCli build --bundles nsis --ci --config 'src-tauri/tauri.bundle.conf.json'
