...
$tauriConfigPath = Join-Path $root 'apps\desktop\src-tauri\tauri.bundle.conf.json'
...
if (@($tauriConfig.bundle.externalBin) -notcontains 'binaries/tiktok-tts-provider') {
    throw 'tauri.bundle.conf.json no declara externalBin binaries/tiktok-tts-provider.'
}
